...
In `__init__()` for your class derived from `LegistarScraper`, overwrite or append to 
...
class MyScraper(LegistarScraper):
    def __init__(self):
        ...
        self.IGNORED_MINUTE_ITEMS[MinutesItem].append(
            "This meeting also constitutes a meeting"
        )
```
...
- Use `legistar_utils.str_simplified()` on string fields to return to the pipeline with clean whitespace characters. e.g. Remove leading and trailing whitespace, and simplify consecutive and mixed whitespace.
